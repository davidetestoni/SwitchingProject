...
namespace IPLookup.Visualization
{
    /// <summary>
    /// Prints a  compressed tree to the console.
    /// </summary>
    public static class CompressedTreePrinter
    {
        class NodeInfo
        {
            public CompressedNode Node;
            public string Text;
            public int StartPos;
        ...
        public static void Print(this CompressedNode root, int topMargin = 2, int leftMargin = 2)
        {
            ...
            {
                var item = new NodeInfo { Node = next, Text = next.NextHop == "" ? $" -- ({next.Segment}) " : $" {next.NextHop} ({next.Segment}) " };
